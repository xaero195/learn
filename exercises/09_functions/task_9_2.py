...
def generate_trunk_config(intf_vlan_mapping, trunk_template, psecurity=None):
    ...
    for key in intf_vlan_mapping:
        finish_list.append(('interface {}'.format(key)).rstrip())
        for item in trunk_template:
            if item.endswith('vlan'):
                finish_list.append((item+' '+str(','.join([str(intf_vlan) for intf_vlan in intf_vlan_mapping[key]])).rstrip()))
            else:
                finish_list.append(item.rstrip())
        if psecurity:
            for item2 in psecurity:
                finish_list.append(item2.rstrip())
    return finish_list
